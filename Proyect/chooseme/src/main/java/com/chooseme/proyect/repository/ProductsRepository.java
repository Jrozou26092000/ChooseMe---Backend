...
public interface ProductsRepository extends CrudRepository<Products, Integer> {
	...
	@Query(value = "SELECT p.product_id, p.brand, p.name, p.created_at, p.active, p.photo, p.price, p.verified, p.description, p.reviewer_id, IFNULL(AVG(s.score),-1) AS score FROM product_types as pt "
		+ "JOIN products as p ON p.product_id = pt.product_id  "
		+ "JOIN types as t ON pt.type_id = t.type_id "
		+ "LEFT JOIN comments as s ON p.product_id = s.product_id "
